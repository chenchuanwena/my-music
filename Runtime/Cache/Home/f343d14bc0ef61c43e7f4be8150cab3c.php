			...
			<aside class="col-sm-4">
				...
				<div class="sidebar_content">	
					<div class="clearfix s_cats">
						<?php $where=array();$where["status"]=1;$_result = M("Genre")->where($where)->limit(20)->select();if($_result):$i=0;foreach($_result as $key=>$vo):$vo['url']=U('/genre/'.$vo['id']);++$i;$mod = ($i % 2 );?><a title="<?php echo ($vo['name']); ?>" target="_genre" <?php if(($vo['id']) == $data['id']): ?>class="active"<?php endif; ?> target="_blank" href="<?php echo ($vo['url']); ?>"><?php echo ($vo['name']); ?></a><?php endforeach; endif;?>
						<a title="全部"  target="_genre" href="<?php echo U('/Genre');?>">全部</a>
					</div>					
				</div>	
