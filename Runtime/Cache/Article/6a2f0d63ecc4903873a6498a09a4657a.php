	...
	<div class="container">
		<div class="row clearfix">
			<aside class="col-sm-3">
				<h2 class="pt_10">关于网站</h2>
				<hr>
				<div class="sidebar_content">
					<ul class="main_menu c_rank">
						<?php $where=array();$where["status"]=1;$where["appname"]="about";$_result = M("site")->alias("__MUSIC")->where($where)->cache(true,86400)->order("create_time ASC")->field("*")->select();if($_result):$i=0;foreach($_result as $key=>$v): $v['url'] = U('/article/site/'.$v['name']);++$i;$mod = ($i % 2 );?><li class="<?php if(($v['name']) == $info['name']): ?>current<?php endif; ?>">
							<a class="mb_0" title="<?php echo ($v['title']); ?>" href="<?php echo ($v['url']); ?>"><?php echo ($v['title']); ?></a>
						</li><?php endforeach; endif;?>
					<ul>
				</div>
				...
				<div class="sidebar_content">
					<ul class="main_menu c_rank">
						<?php $where=array();$where["status"]=1;$where["appname"]="help";$_result = M("site")->alias("__MUSIC")->where($where)->cache(true,86400)->order("create_time ASC")->field("*")->select();if($_result):$i=0;foreach($_result as $key=>$v): $v['url'] = U('/article/site/'.$v['name']);++$i;$mod = ($i % 2 );?><li class="<?php if(($v['name']) == $info['name']): ?>current<?php endif; ?>">
							<a class="mb_0" title="<?php echo ($v['title']); ?>" href="<?php echo ($v['url']); ?>"><?php echo ($v['title']); ?></a>
						</li><?php endforeach; endif;?>
					<ul>
				</div>
				...
				<article class="mb_15">
					<div class="row clearfix mb_15">
						<div class="col-lg-9 col-md-9 col-sm-8">
							<h2 class="mb_5 fw_medium m_xs_bottom_10"><?php echo ($info['title']); ?></h2>
						</div>
					</div>
					<hr class=" mb_20">
					<!--文章-->
					<div class="widget_content mb_15">
