	...
	<div class="container">
		...
		<div class="row clearfix">
			<section class="col-sm-9">
				<div class="info_hd clearfix">	
					<h3 class="hd"><?php echo ($data['title']); ?></h3>
			    </div>
			    <div class="main_list">
			   	
			   		<?php $where=array();$where["status"]=1;$cids = get_stemma($data['id'],M("Category"));if (is_array($cids)){$cids = array_column($cids,"id");array_unshift($cids,$data['id']); $where["category_id"]=array("IN",$cids);}else{ $where["category_id"]=$data['id'];};$_result = M("Document")->alias("__MUSIC")->where($where)->page(!empty($_GET["p"])?$_GET["p"]:1,10)->order("level desc,create_time desc")->field("*")->select();if($_result):$i=0;foreach($_result as $key=>$v): $v['url'] = U('/article/'.$v['id']); $v['cate_url'] = get_category_url($v); $v['cover_url'] = get_cover($v); $v['cate'] = get_category_title($v["category_id"]);++$i;$mod = ($i % 2 );?><a href="<?php echo ($v['url']); ?>" class="info_list_item">
						<?php if(!empty($v['cover_url'])): ?><img alt="<?php echo ($v['title']); ?>" src="<?php echo ($v['cover_url']); ?>"><?php endif; ?>	
						<div>
							<h1><?php echo ($v['title']); ?></h1>
							<p><?php echo (time_format($v['create_time'])); ?></p>
							<span><?php echo (msubstr($v['description'],0,120)); ?>[详细]</span>
						</div>
					</a><?php endforeach; endif; $v_total= M("Document")->where($where)->count();$__PAGE__ = new \Think\Page($v_total,10);$__PAGE__ ->rollPage = 5;$__PAGE__->setConfig("theme","%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%");$__PAGE__->setConfig("prev", "上页");$__PAGE__->setConfig("next", "下页");$v_page= $__PAGE__->show(); ?>			
				</div>
				<div class="row clearfix mb_15">
					<div class="col-sm-8 t_align_c">
						<ul class="horizontal_list clearfix page_list"><?php echo ($v_page); ?></ul>
					</div>
				</div>
			</section>
			...
			<aside class="col-sm-3">
				<div class="side_info_con">
					<h2 class="hd"><?php echo ($data['title']); ?>热点</h2>
					<ul class="side_info_list">
						<?php $where=array();$where["status"]=1;$where["cover_id"]=array("gt","0");$cids = get_stemma($data['id'],M("Category"));if (is_array($cids)){$cids = array_column($cids,"id");array_unshift($cids,$data['id']); $where["category_id"]=array("IN",$cids);}else{ $where["category_id"]=$data['id'];};$_result = M("Document")->alias("__MUSIC")->where($where)->cache(true,86400)->limit("6")->order("view desc")->field("*")->select();if($_result):$i=0;foreach($_result as $key=>$v): $v['url'] = U('/article/'.$v['id']); $v['cate_url'] = get_category_url($v); $v['cover_url'] = get_cover($v); $v['cate'] = get_category_title($v["category_id"]);++$i;$mod = ($i % 2 );?><li><a target="_blank" href="<?php echo ($v['url']); ?>"><img width="74" height="56" border="0" src="<?php echo ($v['cover_url']); ?>" alt="<?php echo ($v['title']); ?>"></a>
							<p>
				...
				<div class="side_info_con"><?php echo hook('Ads','right_up');?></div>
				<div class="side_info_con">
					<div class="top_list ">
						<h2 class="hd">最新推荐</h2>
						<ul>
							<?php $where=array();$where["status"]=1;$_result = M("Document")->alias("__MUSIC")->where($where)->cache(true,86400)->limit("10")->order("level desc,create_time desc")->field("*")->select();if($_result):$li=0;foreach($_result as $key=>$v): $v['url'] = U('/article/'.$v['id']); $v['cate_url'] = get_category_url($v); $v['cover_url'] = get_cover($v); $v['cate'] = get_category_title($v["category_id"]);++$li;$mod = ($li % 2 );?><li class="text-ellipsis"> 
